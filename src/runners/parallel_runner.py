...
class ParallelRunner:
    ...
    def run(self, test_mode=False):
        ...
        n_objectives = len(self.args.env_args['reward_objectives'].strip('[]').split(','))
        # add placeholders to track rewards for objectives
        if n_objectives == 2:
            episode_reward_objs = [None, None]
            if self.args.env_args['momarl_setting'] is True:
                episode_reward_objs = [[0, 0] for _ in range(self.batch_size)]  # Initialize accumulated rewards

        if n_objectives == 3:
            episode_reward_objs = [None, None, None]
            if self.args.env_args['momarl_setting'] is True:
                episode_reward_objs = [[0, 0, 0] for _ in range(self.batch_size)]  # Initialize accumulated rewards
        ...
        while True:
            ...
            for idx, parent_conn in enumerate(self.parent_conns):
                if not terminated[idx]:
                    ...
                    if self.args.env_args['momarl_setting'] is True:
                        if "reward_objective_3" not in data["info"]:
                            episode_reward_objs[idx] = [a + b for a, b in zip(episode_reward_objs[idx],
                                                                         [data["info"]['reward_objective_1'],
                                                                          data["info"]['reward_objective_2']])]
                        else:
                            episode_reward_objs[idx] = [a + b for a, b in zip(episode_reward_objs[idx],
                                                                         [data["info"]['reward_objective_1'],
                                                                          data["info"]['reward_objective_2'],
                                                                          data["info"]['reward_objective_3']])]
        ...
        if self.args.env_args['momarl_setting'] is True:
            cur_objective_rewards = self.test_objective_rewards if test_mode else self.train_objective_rewards
            cur_objective_rewards.extend(episode_reward_objs)
...
def env_worker(remote, env_fn):
    ...
    while True:
        ...
        if cmd == "step":
            ...
            env.reset()  # TODO check what happen here.
            remote.send({
                "state": env.get_state(),
                "avail_actions": env.get_avail_actions(),
                "obs": env.get_obs()
